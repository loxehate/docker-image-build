...
def get_session():
    ...
    try:
        ...
        print(f"模型加载成功，设备: {sess.get_providers()}")
        return sess
    except Exception as e:
        print(f"CUDA 加载失败，回退 CPU: {e}")
...
async_client = openai.AsyncOpenAI(
    api_key="null",
    base_url="http://paddleocr-vlm-server:8080/v1"
)


async def response_paddleocrvl_async(image_b64):
    try:
...
async def detect_text(file: UploadFile = File(...)):
    ...
    try:
        ...
        print(f"EAST 推理耗时: {latency_ms:.2f}ms, 是否有文字: {has_text}")

        if has_text:
            file_data = base64.b64encode(contents).decode("ascii")
            return await response_paddleocrvl_async(file_data)
